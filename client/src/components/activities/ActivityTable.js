...
const ActivityTable = ({ activity }) => {
    ...
    return (
      <Grid container justify="center">
        ...
        <Menu
          ...
          onClose={handleClose}
        >
          <MenuItem>Access Activity</MenuItem>
          <MenuItem>Archive Activity</MenuItem>
        </Menu>
      </Grid>
    );
  }
  ...
  if (workflows) {
    ...
    newRowsArr = rowsArr.map(o => ({
      ...o,
      ...arrInfoMap.get(o.legal_authority)
    }));
    ...
    <Grid>
      <Paper>
        ...
        <Table aria-labelledby="activitiesTable">
          ...
          />
          <TableBody>
            {stableSort(newRowsArr, getSorting(order, orderBy)).map(
              (row, index) => {
                if (row.wait_time > 7) {
                  var wtColor = "wt-red";
